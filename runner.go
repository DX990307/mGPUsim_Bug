...
var busyTimeTracerFlag = flag.Bool("report-busy-time", false, "Report SIMD Unit's busy time")
...
func (r *Runner) ParseFlag() *Runner {
	...
	if *busyTimeTracerFlag {
		r.ReportBusyTime = true
	}
...
//------------------------------------------------------------------------------
func (r *Runner) addBusyTimeTracer() {
	if !r.ReportBusyTime {
		return
	}
	...
	for _, gpu := range r.platform.GPUs {
		for _, cu := range gpu.CUs {
			...
			tracing.CollectTrace(cu.(tracing.NamedHookable), perSIMDBusyTimeTracer)
		}
	}
}
...
func (r *Runner) reportBusyTime() {
	for _, t := range r.simdBusyTimeTracers {
		r.metricsCollector.Collect(
			t.cu.Name(), "busy_time", float64(t.tracer.BusyTime()))
	}
}
